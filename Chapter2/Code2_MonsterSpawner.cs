...
public class MonsterSpawner : MonoBehaviour
{
    ...
    // 유니티 에디터 상에서 보이는 헤더이다.
    [Tooltip("몬스터 프리팹")]
    // 유니티 에디터 상에 보이도록 혹은 저장 등을 위해 직렬화할 때 사용한다.
    [SerializeField]
    private GameObject _monster;
    // 오브젝트가 최초로 깨어났을 때 호출된다.
    private void Start()
    {
        for (int i = 0; i < COUNT_TO_SPAWN; i++)
        {
            ...
            // 몬스터를 position에 Quaternion.identity(회전 없음)각도로 스폰한다.
